...
public class SetLimitCMD extends FreedomCommand
{
    ...
    public void run(CommandSender sender, String[] args)
    {
        Bukkit.getOnlinePlayers().forEach(p -> {
            getPlugin().getWorldEditBridge().setLimit(p, 8000);
        });
        ...
        if (!(sender instanceof Player))
        {
            ...
            {
                bcastMsg("§cFreedom - Setting default WorldEdit limit to 8000");
            } else {
                bcastMsg("§c" + sender.getName() + " - Setting default WorldEdit limit to 8000");
            }
        } else {
            bcastMsg("§c" + sender.getName() + " - Setting default WorldEdit limit to 8000");
        }
